...
public class LazyParsable<T> {
  ...
  public static class RawDeserializer extends JsonDeserializer<Object> {
    ...
    public Object deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
      ...
      //  1. If getTokenLocation()/getCurrentLocation().getCharOffsett() returns -1, the source reference is not a string, but an input
      //   stream, byte array, etc. and the value could be still extracted as a string efficiently.
      //  2.  If getTokenLocation()/getCurrentLocation().getCharOffsett() larger than 0, but the location doesn't point to the position of
      //  the token, then it is possible to extract the value in some cases. For the rest( e.g. input stream without mark support,
      //  etc. ) the value must be parsed.
