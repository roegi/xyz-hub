...
public class ServiceHealthCheck extends ExecutableCheck {
	...
	private Response parseIfPossible(String serviceResponse) {
		...
		try {
			...
			//It's no JSON
			r = new Response();
			r.setMessage(serviceResponse.length() > 512 ? serviceResponse.substring(0, 512 - 4) + " ..." : serviceResponse);
			return r;
		}
	}
	...
	private Status doRequest(Status status, URL url) {
		try {
			...
			Response response = parseIfPossible(IOUtils.toString(conn.getInputStream(), Charset.defaultCharset()));
			...
			if (responseCode >= 200 && responseCode <= 299) {
				...
				if (status.getResult() == WARNING)
					previousWarnMessage = (String) status.getAdditionalProperties().get("warnMessage") + "\n";
	...
	private URL getTargetLocation(HttpURLConnection conn) throws MalformedURLException {
		...
		if (targetLocation == null)
			targetLocation = conn.getHeaderField("location");
