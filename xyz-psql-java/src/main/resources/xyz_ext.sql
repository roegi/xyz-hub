...
CREATE OR REPLACE FUNCTION xyz_qk_grird(lev integer)
 RETURNS TABLE(qk text,qk_bbox geometry) AS
$BODY$
DECLARE
	width integer := pow(2,lev);
	tileX integer := 0;
	tileY integer := 0;
BEGIN
	LOOP
		LOOP
			SELECT xyz_qk_lrc2qk(tileY,tileX,lev) into qk;
			SELECT xyz_qk_qk2bbox(qk) into qk_bbox;
			RETURN NEXT;
			tileX := tileX + 1 ;
			EXIT WHEN tileX = width;
		END LOOP;
		tileX := 0;
		tileY := tileY + 1 ;
		EXIT WHEN tileY = width;
	END LOOP;
END;
$BODY$
